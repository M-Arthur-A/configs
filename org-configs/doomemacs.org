...
- [[#sidebar][SIDEBAR]]
  - [[#general][General]]
  - [[#sidebar-1][Sidebar]]
  - [[#toggling-sidebar][Toggling sidebar]]
...
- [[#link-open-in-browser][LINK OPEN IN BROWSER]]
- [[#lsp-mode][LSP-MODE]]
  - [[#change-tab-indent-for-c-languages][change tab indent for C languages]]
  - [[#run-executable-c][run executable c++]]
  - [[#debug-c][debug c++]]
  - [[#python-lsp-pyright][python lsp-pyright]]
  - [[#use-poetry][use poetry]]
  - [[#run-python-pudb-debugger-in-poetry-env][run python pudb debugger in poetry env]]
  - [[#tramp-register-lsp-mode][tramp register lsp-mode]]
  - [[#ruff-python-linter-with-flymake-archive][ruff python linter with flymake (ARCHIVE)]]
  - [[#ruff-python-linter-with-flycheck][ruff python linter with flycheck]]
- [[#modeline][MODELINE]]
...
- [[#registers-and-recentf][REGISTERS and RECENTF]]
- [[#spell-check][SPELL CHECK]]
  - [[#spell-fu-better][spell-fu (better)]]
  - [[#flyspell-mode-deprecated][flyspell-mode (deprecated)]]
- [[#shells][SHELLS]]
- [[#splits][SPLITS]]
...
=C-c n s   : Toggle navigation sidebar
...
(define-minor-mode my/config-mode
  ...
  :keymap (let ((map (make-sparse-keymap)))
            ...
            (bind-key "C-c n s"   #'my/org-sidebar-toggle map)
...
List of keybindings (SPC h b b)")
  ...
  (setq dashboard-startup-banner "~/.config/doom/doom-emacs-logo.txt")  ;; use custom text as banner
...
;; (custom-set-faces!
;;   '(font-lock-comment-face :family "Source Code Pro" :slant italic)
;;   '(font-lock-keyword-face :family "Source Code Pro" :slant italic))
(setq doom-font (font-spec :family "Hack" :size 28)
      doom-variable-pitch-font (font-spec :family "Hack" :size 28)
      doom-big-font (font-spec :family "Hack" :size 36))
(after! doom-themes
  (setq doom-themes-enable-bold t
        doom-themes-enable-italic t))
(custom-set-faces!
  '(font-lock-comment-face :family "Hack" :slant italic)
  '(font-lock-keyword-face :family "Hack" :slant italic))
#+END_SRC
...
#+end_src
* LSP-MODE
** change tab indent for C languages
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
(use-package cc-mode
  :init
...
#+end_src
** run executable c++
~SPC c .~ to run executable from bin folder of current project
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
(defun cpp-project-exe-run ()
  (shell-command (expand-file-name
                  "bin/*"
                  (file-name-directory (buffer-file-name)))))
(map! :leader
      :desc "Execute from ./bin/*" "c ." #'(lambda () (interactive) (cpp-project-exe-run)))
#+end_src
** debug c++
in =init.el= in :tools add (debugger +lsp)
or just debugger withou +lsp
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
...
#+end_src
** python lsp-pyright
first add +pyright to python row in init.el
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
...
;;           ;; (setq lsp-pyright-python-executable-cmd "/opt/anaconda/bin/python")))
;;           (setq lsp-pyright-python-executable-cmd "/home/arthur/.local/bin/poetry -C /home/arthur/....../emacs-env/ run python")))
#+end_src
** use poetry
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
;; (use-package poetry
;;  :ensure t)
(map! :leader
      :desc "Run poetry for python" "o ." #'(lambda () (interactive) (poetry)))
#+end_src
** run python pudb debugger in poetry env
running in alacritty new window asynchronously
kbd: =SPC c p=
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
;;(setq emacs-env-dir "/home/analyst/Code/Python/OtherTools/emacs-env/")
;;(string-replace emacs-env-dir "" (file-name-directory buffer-file-name))
(defun pydebug ()
    (interactive)
    (start-process "my-process" "pudb-debugger" "/usr/bin/alacritty" (concat "--working-directory=" (file-name-directory buffer-file-name)) "-e" "poetry" "run" "pudb" (file-name-nondirectory buffer-file-name))
)

(defun pylaunch ()
    (interactive)
    (start-process "my-process" "poetry-python" "/usr/bin/alacritty" (concat "--working-directory=" (file-name-directory buffer-file-name)) "--hold" "-e" "poetry" "run" "python" (file-name-nondirectory buffer-file-name))
)

(map! :leader
      :desc "Run in terminal" "c p" #'(lambda () (interactive) (pylaunch)))
(map! :leader
      :desc "Debug with pudb" "c P" #'(lambda () (interactive) (pydebug)))
#+end_src

** tramp register lsp-mode
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
(after! tramp
...
#+end_src
** ruff python linter with flymake (ARCHIVE)
before - ~straight-use-package~ then ~flymake-ruff~
#+begin_src emacs-lisp
...
#+end_src
** ruff python linter with flycheck
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
(require 'flycheck)
(setq flycheck-python-ruff-executable "/usr/bin/ruff")
...
(map! :leader
      (:prefix ("=" . "open file")
       :desc "Edit agenda file" "a" #'(lambda () (interactive) (find-file "~/Project/notes/agenda.org"))
...
=NOTE:= I have the location of my Org directory and Roam directory in $HOME/Project/Notes/roam/ which is a Nextcloud folder that allows me to instantly sync all of my Org work between my home computer and my office computer.

#+BEGIN_SRC emacs-lisp :tangle /home/arthur/.config/doom/config.el
(map! :leader
      :desc "Org babel tangle" "m B" #'org-babel-tangle)
(after! org
  (setq org-directory "~/Project/Notes/roam/"
        org-agenda-files '("~/Project/Notes/roam/agenda.org")
...
** Org-journal
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
(setq org-journal-dir "~/Project/Notes/roam/journal/"
...
(setq org-publish-project-alist
      '(("org-static"
         :base-directory "~/Project/Notes/web/html/"
         :base-extension "css\\|js\\|png\\|jpg\\|gif\\|pdf\\|mp3\\|ogg\\|swf"
         :publishing-directory "~/Project/Notes/web/public_html/"
         :recursive t
         :exclude ".*/Project/Notes/web/org-html-themes/.*"
...
#+begin_src emacs-lisp :tangle /home/arthur/.config/doom/config.el
;; (after! org-roam
  ;; (setq org-roam-directory "~/Project/Notes/roam"))
;; (use-package org-roam
;;   :ensure t
;;   :init
;;   (setq org-roam-v2-ack t)
;;   :custom
;;   (org-roam-directory "~/Project/Notes/roam")
...
(map! :leader
      (:prefix ("r" . "registers")
       :desc "Open recent files" "." #'recentf-open
...
(add-hook 'spell-fu-mode-hook
  (lambda ()
    ...
    (spell-fu-dictionary-add
      (spell-fu-get-personal-dictionary "en-personal" "/home/arthur/.aspell.en.pws"))
    (spell-fu-dictionary-add
      (spell-fu-get-personal-dictionary "ru-personal" "/home/arthur/.aspell.ru.pws"))))
#+end_src
** flyspell-mode (deprecated)
#+begin_src
(global-set-key
 [f5]
 (lambda ()
