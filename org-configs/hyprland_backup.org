...
# Float Qalculate! (однострочный вариант)
windowrule = float on, match:class ^(io.github.Qalculate.qalculate-qt)$, match:title ^(Qalculate!)$

# Игнорировать запросы maximize от приложений (блочный вариант)
windowrule {
...
# Исправить некоторые проблемы с перетаскиванием в XWayland (блочный вариант)
windowrule {
...
# Scratchpads (однострочный вариант)
windowrule = float on, match:class ^(scratchpad)$, match:title ^(alacritty)$
...
bind = $mainMod, V, exec, vicinae deeplink vicinae://launch/clipboard/history
...
binde = , v, exec, /home/arthur/Applications/wofi-vpn.sh
